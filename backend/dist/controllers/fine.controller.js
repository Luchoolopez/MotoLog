...
class FineController {
    static async create(req, res) {
        try {
            ...
            const data = req.body;
            const record = await fine_service_1.FineService.create(data, req.user.id);
            res.json(record);
        }
        catch (error) {
    ...
    static async getByMoto(req, res) {
        try {
            ...
            const records = await fine_service_1.FineService.getAllByMotoId(Number(motoId), req.user.id);
            res.json(records);
        }
        catch (error) {
    ...
    static async update(req, res) {
        try {
            const { id } = req.params;
            const updated = await fine_service_1.FineService.update(Number(id), req.body, req.user.id);
            res.json(updated);
        }
        catch (error) {
    ...
    static async delete(req, res) {
        try {
            const { id } = req.params;
            await fine_service_1.FineService.delete(Number(id), req.user.id);
