...
class MaintenanceHistoryService {
        ...
        try {
            ...
            if (consumed_items && Array.isArray(consumed_items) && consumed_items.length > 0) {
                ...
                for (const [itemIdStr, cantidadUsada] of Object.entries(grouped)) {
                    ...
                    const warehouseItem = await warehouse_item_model_1.WarehouseItem.findOne({
            ...
            const moto = await motorcycle_model_1.Motorcycle.findOne({
    ...
    async getHistoryByMotoId(motoId, userId) {
    ...
    async getById(id, userId) {
        const record = await maintenance_history_model_1.MaintenanceHistory.findOne({
            where: { id },
            include: [{
    ...
    async ensureMotoBelongsToUser(motoId, userId) {
