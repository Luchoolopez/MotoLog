...
const SeguroPage = () => {
    const [seguros, setSeguros] = useState<LicenseInsurance[]>([]);
    ...
    const [formData, setFormData] = useState<CreateLicenseInsuranceDto>({
        moto_id: 0,
        tipo: 'Seguro',
        entidad: '', // Compañía de Seguro
        nro_documento: '', // Póliza ID (optional display)
        fecha_vencimiento: '',
        monto: 0,
        cobertura: '',
        pagado: false,
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const filtered = data.filter((d: LicenseInsurance) => d.tipo === 'Seguro');
            setSeguros(filtered);
        } catch (error) {
            console.error(error);
            showToast('Error al cargar seguros', 'error');
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!formData.entidad) {
            showToast('Debes ingresar la Compañía de Seguro', 'error');
            return;
        }
        ...
        try {
            ...
            if (isEditing && editId) {
                ...
                showToast('Seguro actualizado', 'success');
            } else {
                await LicenseInsuranceService.create(payload as any);
                showToast('Seguro registrado', 'success');
            }
            setShowModal(false);
            fetchData();
            resetForm();
        ...
        <div className="container-fluid flex-grow-1" style={{
                            ...
                            <Table hover variant="dark" className="mb-0 text-white">
                                <thead className="text-uppercase text-white-50 small bg-black">
                                    <tr>
                                        ...
                                        <th className="py-3 bg-black" style={{ position: 'sticky', top: 0, zIndex: 1000 }}>Compañía</th>
                                        <th className="py-3 bg-black" style={{ position: 'sticky', top: 0, zIndex: 1000 }}>Cobertura</th>
                                ...
                                <tbody>
                                    {loading ? (
                                        ...
                                        <tr><td colSpan={7} className="text-center py-5 text-muted">No hay registros de seguros</td></tr>
                                    ) : (
                                        seguros.map(item => (
                                            <tr key={item.id} className="align-middle">
                                                ...
                                                <td>{item.entidad}</td>
                                                <td><Badge bg="info" className="fw-normal">{item.cobertura || '-'}</Badge></td>
                                ...
                                seguros.map(item => (
                                    <Card key={item.id} className="mb-3 bg-secondary bg-opacity-25 border-secondary">
                                        <Card.Body>
                                            ...
                                            <div className="d-flex flex-column gap-1 small text-white-50 mb-3">
                                                <div className="d-flex justify-content-between">
                                                    <span>Cía: {item.entidad}</span>
                                                    <Badge bg="info" className="fw-normal">{item.cobertura || '-'}</Badge>
                                                </div>
                                            </div>
            ...
            <Modal show={showModal} onHide={() => setShowModal(false)} centered contentClassName="bg-dark text-white border border-secondary shadow-lg">
                <Modal.Header closeButton closeVariant="white" className="border-secondary">
                    <Modal.Title>{isEditing ? 'Editar Seguro' : 'Registrar Seguro'}</Modal.Title>
                </Modal.Header>
                <Form onSubmit={handleSubmit}>
                    <Modal.Body>
                        <Row className="g-3">
                            ...
                            <Col md={6}>
                                <Form.Group>
                                    <Form.Label>Tipo Cobertura</Form.Label>
                                    <Form.Control
                                        type="text"
